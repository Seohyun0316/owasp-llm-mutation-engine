...
본 문서는 LLM Greybox Fuzzer의 Mutation Engine에서 사용되는 **Mutation Operator**의 공통 계약(contract)을 정의한다.  
목표는 다음이다.
...
- 본 계약 버전은 `v0.1`이다.
- `apply()` 시그니처, 상태 코드(OK/SKIPPED/INVALID), trace 최소 스키마 변경은 **breaking change**로 간주한다.
...
- 강도 의미는 연산자별로 문서화해야 한다.
  - 예: 공백 교란 연산자의 strength=1은 소량, strength=5는 대량 교란
...
- `SKIPPED`
  ...
  - `child_text`는 **원본(`seed_text`) 그대로 반환**을 권장한다.
- `INVALID`
